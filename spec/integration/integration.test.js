...
const startService = serviceName =>
  new Promise((resolve, reject) => {
    ...
    const child = spawn(
      `bundle exec appraisal ${appraisalName} bundle config set --local path vendor/bundle && bundle && ruby ./example/${serviceName}.rb --test`,
      [],
      { shell: true },
    );
...
beforeAll(async () => {
  serviceProcesses = await Promise.all(serviceList.map(({ name }) => startService(name))).catch(
    console.error,
  );
